...
export function validateSchema(schema) {
  ...
  if (!schema.parts || schema.parts.length === 0) {
    ...
    warnings.push(`Schema has only ${schema.parts.length} parts (recommended 3-10)`)
  } else if (schema.parts.length > 10) {
    warnings.push(`Schema has ${schema.parts.length} parts (max 10), lower priority parts may be dropped`)
  }
  ...
  const validGeometries = ['Box', 'Sphere', 'Cylinder', 'Cone', 'Torus', 'Lathe', 'Tube']
  for (const part of schema.parts) {
